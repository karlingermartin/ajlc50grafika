...
void search_closest_car(World *world, Vertex camera_position)
{
	int car_id;
	...
	{
		float distance = sqrt(pow((camera_position.x - world->cars[i].position.x), 2) + pow((camera_position.z - world->cars[i].position.z), 2));
...
void keyboardUp(unsigned char key, int x, int y)
{
	...
	case 'x':
		if (move_car_id == -1)
		{
			search_closest_car(&world, camera.position);
		}
		else
		{
			move_car_id = -1;
	...
	case 'q':
		if (move_car_id != -1)
		{
			world.cars[move_car_id].rotation += 30.0f;
		}
		break;
	case 'e':
		if (move_car_id != -1)
